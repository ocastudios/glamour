...
class Button(Updatable):
    ...
    def __init__(
        ...
        if self.type_of_button == "text":
            ...
            self.fontA = pygame.font.Font(
                os.path.join(directory.fonts, settings.main_font),
        ...
        self.pos = [
            (self.position[0] - (self.image.get_size()[0] / 2)),
    ...
    def update_all(self):
        ...
        try:
            self.pos = [
                self.level_pos[0] + self.position[0] - (self.size[0] / 2),
    ...
    def click_detection(self):
        if (
            ...
            and self.universe.pointer.type == 2
        ) or (
...
class Letter(GameText):
    ...
    def click_detection(self):
        if -0.5 < self.universe.level.speed < 0.5:
                    ...
                    if self.key == "Spacebar":
                        ...
                        while True:
                            if menu.princess.name.text in saved_games:
                                counting += 1
                                menu.princess.name.text = random.choice(
                                    random_names.names
                                )
                            else:
                                break
    ...
    if format == "image":
            ...
            if not exception or part not in exception:
                if row and row[part] and row[part] not in ("None", 0, "0"):
                    img = pygame.image.load(
                        os.path.join(directory.princess, row[part], filename)
                    ).convert_alpha()
                ...
                if exception[part] and exception[part] not in ("None", 0, "0"):
                    ...
                    img = pygame.image.load(
                        os.path.join(directory.princess, garment, filename)
                    ).convert_alpha()
                    big_image.blit(img, (0, 0))
        if princess == "princess_garment":
            ...
            if dirt > 0:
                big_image.blit(
                    pygame.image.load(
                        os.path.join(directory.princess, "dirt" + str(dirt), filename)
                    ).convert_alpha(),
                    (0, 0),
                )
            ...
            if not exception or part not in exception:
                ...
                if exception[part] and exception[part] not in ("None", 0, "0"):
                    if part == "hair_back":
                        ...
                        garment = part + "_" + exception[part]
                    img = utils.img.image(
                        os.path.join(directory.princess, garment, filename), invert=flip
                    )
                else:
                    img = None
            result.append(img)
