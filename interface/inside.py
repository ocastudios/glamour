...
class Inside():
	def __init__(self, level, item_type, item_list):
		...
		self.locked = {'geisha': database.query.is_locked(self.level,'face','geisha'),
						'indian': database.query.is_locked(self.level,'dress','indian'),
						'crystal': database.query.is_locked(self.level,'shoes','crystal')
		}
...
#		self.level.universe.click = False
	...
	def all_set(self):
		...
		if self.locked['indian']:
			my_outfit = database.query.my_outfit(self.level,"princess_garment")
			geisha_garments = 0
			for i in my_outfit:
				if ("geisha" in str(i)) or ('kimono' in str(i)) or ("flower" in str(i)):
					geisha_garments+= 1
			if geisha_garments >= 3:
				self.level.unlocking = {'type':'dress','name':'indian'}
		...
		if self.locked['crystal']:
			if my_outfit[9] == 'accessory_ribbon' and \
				...
				self.level.unlocking = {'type':'shoes','name':'crystal'}
...
class BigPrincess():
	def __init__(self, room, pos = "left"):
		...
		row = database.query.my_outfit(room.level, 'princess_garment')
		self.image_dict = {
						"hair_back" :0,
						"skin"	  :1,
						"face"	  :2,
...
class Princess_Home():
	def __init__(self, level, princess=None):
		...
		row = database.query.my_outfit(self.level,princess_name)
...
class Home():
	def __init__(self, level):
		...
		self.big_princess = BigPrincess(self)
		self.past_balls = []
		last_balls = database.query.last_balls(level)
		if len(last_balls)>=1:
			for i in last_balls:
		...
		self.locked = database.query.is_locked(self.level,'face','lipstick')
		self.chosen_number = 0
		self.menu = [(i.pos[0]+(i.size[0]/4),i.pos[1]+(i.size[1]/4)) for i in self.buttons if i.__class__== widget.Button]
	...
	def all_set(self):
		self.status = 'done'
		if self.locked:
			self.level.unlocking={'type':'face','name':'lipstick'}
			self.locked = False
