...
class Inside:
    def __init__(self, universe, item_type, item_list):
        ...
        if item_type != "shower":
            ...
            self.buttons = (
                widget.Button(
                    self.universe, directory.button_ok, (410, 530), [0, 0], self.all_set
                ),
                self.chosen_glow,
            )
            self.menu.extend(
            ...
            universe.stage.big_princess = (
                universe.stage.big_princess
                or widget.princess_image(self.universe, "princess_garment")
            )
        else:
            self.items = []
            shower = widget.GameText(self.universe, t("Take a shower"), (360, 400))
            ok_pos = (
                d(shower.pos[0] + (shower.size[0] / 2)),
                d(shower.pos[1] + (shower.size[1])) + 50,
            )
            ok_button = widget.Button(
                self.universe, directory.button_ok, ok_pos, [0, 0], self.clean_up
            )
            quit = widget.GameText(self.universe, t("Leave"), (1080, 400))
            cancel_pos = (
                d(quit.pos[0] + (quit.size[0] / 2)),
                d(quit.pos[1] + (quit.size[1])) + 50,
            )
            cancel_button = widget.Button(
                self.universe, directory.button_cancel, cancel_pos, [0, 0], self.all_set
            )
            title = widget.GameText(
                self.universe, t("Would you like to take a shower?"), (720, 100)
            )
        ...
        self.locked = {
            "geisha": database.query.is_locked(self.universe, "face", "geisha"),
    ...
    def all_set(self):
        ...
        if self.chosen_item:
            exec(
                ...
                + self.type_of_items
                + "_"
                + self.chosen_item.name
                + '")'
            )
        else:
            save.save_file(self.universe)
        self.universe.level.princesses[0] = princess.Princess(
            self.universe, INSIDE=True
        )
    ...
    def __init__(self, room, name, index):
        ...
        if self.type != "shower":
            self.image = utils.img.image(
                os.path.join(
                    directory.princess, self.type + "_" + name, "big_icons", "0.png"
                )
            )
            self.size = self.image.get_size()
            self.pos = [
                p(500, r=0) + (index * (self.size[0])),
                (self.universe.height / 2) - (self.size[1] / 2),
            ]
            self.speed = 1
    ...
    def click_detection(self):
        if self.rect.colliderect(self.universe.pointer.rect):
            ...
            if self.universe.click:
                self.room.chosen_item = self
                self.room.chosen_glow.pos = self.pos[0] - p(40, r=0), self.pos[1] - p(
                    40, r=0
                )
                self.universe.stage.big_princess = widget.princess_image(
                    self.universe, "princess_garment", exception={self.type: self.name}
                )
...
class Princess_Home:
    def __init__(self, universe, princess=None):
        ...
        self.princess_icon = utils.img.image(
            os.path.join(directory.ball, princess["icon"])
        )
        self.status = "outside"
        self.universe = universe
        mymessage = messages.princesses_phrases[
            random.randint(0, (len(messages.princesses_phrases) - 1))
        ]
        if "%s" in mymessage:
        ...
        self.buttons = (
            widget.Button(
                self.universe, directory.button_ok, (410, 450), [0, 0], self.all_set
            ),
        ...
        self.universe.stage.big_princess = (
            self.universe.stage.big_princess
            or widget.princess_image(self.universe, "princess_garment")
        )
        self.chosen_number = 0
    ...
    def __init__(self, universe):
        ...
        self.buttons = (
            ...
            widget.GameText(
                self.universe,
                t(
                    "It is not very cute to repeat your outfits. Check out what you wore at past Balls and try to find something different."
                ),
                (720, 750),
                box=(600, 300),
            ),
                ...
                for ii in sorted(list(image_dict.items()), key=operator.itemgetter(1)):
                    if i[ii[0]] and i[ii[0]] != "None":
                        image.blit(
                            utils.img.image(
                                os.path.join(
                                    directory.princess, i[ii[0]], "stay", "0.png"
                                )
                            ),
                            (0, 0),
                        )
