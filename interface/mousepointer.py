...
class MousePointer():
    def __init__(self,level, type = 1):
        ...
        if type == 2:
            self.type = 2
            self.images         = utils.img.OneSided(main_dir+'/data/images/interface/mouse-icon/pointer2/')
            self.image          = self.images.list[self.images.number]
            self.size           = self.image.get_size()
            self.pos            = self.mouse_pos
        else:
            self.type = 1
            self.images         = utils.img.OneSided(main_dir+'/data/images/interface/mouse-icon/pointer/')
            self.image          = self.images.list[self.images.number]
            self.size           = self.image.get_size()
