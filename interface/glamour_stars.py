...
class Glamour_Stars:
    def __init__(self, universe):
        ...
        self.image_lists = [
            utils.img.OneSided(os.path.join(directory.star, "star" + i))
            for i in ("0", "1", "2", "3")
        ]
...
class Lil_Stars:
    def __init__(self, universe, pos):
        self.universe = universe
        self.rotating = utils.img.There_and_back_again(
            os.path.join(directory.interface, "lil_star", "right"),
            second_dir=os.path.join(directory.interface, "lil_star", "left"),
        )
...
class Lil_Star_Back:
    def __init__(self, universe, pos):
        ...
        self.image = utils.img.image(
            os.path.join(directory.interface, "lil_star", "back.png")
        )
        self.pos = p(pos)
