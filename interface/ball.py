...
class Ball:
    def __init__(self, universe, princess):
        ...
        self.background = pygame.Surface(
            (universe.width, universe.height), pygame.SRCALPHA
        ).convert_alpha()
        universe.level.loading()
        for file in (
            os.path.join(directory.ball, "base.png"),
            os.path.join(directory.ball, "back-bubbles.png"),
        ):
            self.background.blit(
                utils.img.scale_image(pygame.image.load(file).convert_alpha()), (0, 0)
            )
            universe.level.loading()
        ...
        pygame.mixer.music.load(
            os.path.join(directory.music, "strauss_waltz_wedley.ogg")
        )
        universe.level.loading()
        ...
        if self.boyfriend:
            self.dancers.append(
                NewDancer(universe, "princess_garment", self.boyfriend.hard_name)
            )
        universe.level.loading()
    ...
    def update_all(self):
        ...
        if self.boyfriend:
            ...
            self.texts += [
                widget.GameText(
                    self.universe, t("...but no one's heart yet."), (1090, 237), [0, 0]
                )
            ]
    ...
    def compute_glamour_points(self, universe):
        ...
        fairy_tale_rows = [
            cursor.execute("SELECT * FROM " + princess_name).fetchall()
            for princess_name in (
                "cinderella",
                "rapunzel",
                "sleeping_beauty",
                "snow_white",
        ...
        princess_garments = {
            "this_ball": [
                princess_rows[len(princess_rows) - 1][item] for item in garments
            ],
            "last_ball": [
                princess_rows[len(princess_rows) - 2][item] for item in garments
            ],
        }
        others_garments = {"this_ball": [], "last_ball": []}
        ...
        present_repetitions = sum(
            [
                others_garments["this_ball"].count(i)
                for i in princess_garments["this_ball"]
            ]
        )
        last_ball_garments = (
            others_garments["last_ball"] + princess_garments["last_ball"]
        )
        past_repetitions = sum(
            [last_ball_garments.count(i) for i in princess_garments["this_ball"]]
        )
        my_garments_history = []
        for item in garments:
        ...
        history_repetitions = sum(
            [my_garments_history.count(i) for i in princess_garments["this_ball"]]
        )
        ...
        # Fashion Points
        print(
            "Fashion points is equal to 30 - 3*repetition_points - past_repetition_points"
        )
        ...
        if (
            ...
            print(
                "	You cannot earn glamour points unless you visit other streets in order to update your outfit."
            )
        ...
        print(
            ...
            + str(accumulated_points + glamour_points)
            + " glamour points."
        )
        ...
        universe.level.panel[1] = widget.GameText(
            universe, str(total_points), (1000, 30), [0, 0], color=(58, 56, 0)
        )

    def return_to_game(self):
        self.Frame.set_next_ball_clothes()
        if self.universe.level.princesses[0].points >= settings.boyfriend_rank[-1][
            1
        ] and not database.query.won(self.universe):
            ...
            self.universe.level.BathhouseSt(
                goalpos=round(5220 * scale), clean_princess=True
            )
            if self.locked:
...
class BallFrame:
    def __init__(self, ball):
        ...
        self.ball = ball
        princesses = [
            FairyTalePrincess(self, i[0], i[1], i[2], i[3], iconpos=i[5], name=i[4])
            for i in (
                [
                    (130, 150),
                    "hair_snowwhite",
                    "pink",
                    "princess-icon-apple.png",
                    "Snow_White",
                    220,
                ],
                [
        ...
        past_princesses = [
            FairyTalePrincess(self, i[0], i[1], i[2], name=i[3], ball=1)
            for i in (
...
class FairyTalePrincess:
    def __init__(
        ...
        self.image = utils.img.scale_image(
            pygame.Surface((200, 200), pygame.SRCALPHA).convert_alpha()
        )
        ...
        if name != "princess_garment":
            if icon:
                ...
                self.symbolpos = int(
                    round(iconpos * scale - (float(self.symbol.get_width()) / 2))
                )
        self.pos = [self.position[0], self.position[1]]
        self.image = widget.princess_image(
...
class StarBall:
    def __init__(self):
        self.images = utils.img.MultiPart(
            [
                os.path.join(directory.ball, "star-score", "backlight"),
...
class BoyFriend:
    def __init__(self, universe, points):
        print(
            "Oh my! You are so beautiful that most certainly someone will fall for you tonight!"
        )
        boyfriend = None
        ...
        self.image = utils.img.image(
            os.path.join(directory.boyfriends, boyfriend, "0.png")
        )
        self.pos = p([1000, 298])
...
class NewDancer:
    ...
    def __init__(self, universe, princess_name=None, boyfriend=None):
        ...
        ordered_directory_list = [
            os.path.join(directory.boyfriends, boyfriend, "dance", "body")
        ]
        ...
        for i in ("skin", "dress", "hair", "accessory"):
            ...
            if part:
                ordered_directory_list.append(
                    os.path.join(directory.princess, players_outfit[i], "dance")
                )
        ordered_directory_list.append(
            os.path.join(directory.boyfriends, boyfriend, "dance", "head")
        )

        self.images = utils.img.MultiPart(
            ordered_directory_list, loading=universe.level.loading
        )
