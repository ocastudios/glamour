...
class Sky():
	...
	def __init__(self,universe):
		...
		self.image = utils.img.image(os.path.join(directory.skies,'daytime','daytime.png'))
		self.pos = (0,0)
	...
	def update_all(self):
		if self.universe.level.clock[1].time in ('evening', 'night','ball'):
			...
			if self.universe.level.clock[1].count%2== 0 and self.universe.level.clock[1].count != self.prev:
