    ...
    def __init__(self, center_distance, dir, universe, invert=False, height=False):
        ...
        if not height:
            self.pos = [
                ...
                round(universe.level.floor - (self.size[1] - round(15 * scale))),
            ]
        else:
            self.pos = [
                (self.center_distance),
                round(universe.level.floor - self.size[1] - (height)),
            ]
        ...
        try:
            self.lights = {
                "images": utils.img.OneSided(os.path.join(light_directory, light)),
                "status": "off",
                "position": self,
            }
        except:
            ...
            if not princess.inside:
                if princess.action[0] == "open_door":
                    ...
                    self.universe.stage.big_princess = widget.princess_image(
                        self.universe, "princess_garment"
                    )
                    self.open = True
        else:
            self.open = False
            ...
            for i in self.universe.level.gates:
                if i.__class__ == Gate or i.__class__ == BuildingDoor:
                    if i.rect.colliderect(princess.rect):
                        control += 1
                        self.pos = (
                            i.pos[0] + (i.size[0] / 2 - (self.size[0] / 2)),
                            i.pos[1] - round(150 * scale),
                        )
                        self.image = self.images.list[self.images.number]
...
class Cloud:
    ...
    def __init__(self, universe):
        self.universe = universe
        self.pos = (
            random.randint(0, int(universe.width)),
            random.randint(0, int(universe.height / 4)),
        )
        self.nimbus = self.nimbus or [
            utils.img.image(os.path.join(directory.nimbus, i, "0.png"))
            for i in ("0", "1", "2", "3")
        ]
