...
class Water(Floor):
    ...
    sound = pygame.mixer.Sound(
        os.path.join(settings.directory.sounds, "scenario", "water", "Wavesloop.ogg")
    )
    ...
    def update_all(self):
        ...
        self.pos = (
            self.universe.center_x + (self.center_distance),
            self.universe.floor - self.height,
        )
...
class Bridge:
    def __init__(self, dir, index, universe, main=True):
        if main:
            self.images = utils.img.OneSided(os.path.join(dir, "main"))
        else:
            self.images = utils.img.OneSided(dir)

        if main:
            self.left_bank = Bridge(
                os.path.join(str(dir), "left_bank"), index - 1, universe, main=False
            )
            self.right_bank = Bridge(
                os.path.join(str(dir), "right_bank"), index + 1, universe, main=False
            )
        ...
        width = p(400, r=False)
        if main:
            self.center_distance = round((width * index) - width)
        else:
            self.center_distance = round(width * index)
