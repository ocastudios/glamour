...
def princess_garment(
    ...
    cursor.execute(
        "UPDATE save SET center_distance = '"
        + str(int(Princess.center_distance / scale))
        + "' WHERE name = '"
        + Princess.name
        + "'"
    )
    if hair:
        cursor.execute(
            "UPDATE princess_garment SET hair_back = '"
            + str(hairback)
            + "' WHERE id = (SELECT max(id) FROM princess_garment)"
        )
    if skin:
        cursor.execute(
            "UPDATE princess_garment SET skin	= '"
            + str(skin)
            + "' WHERE id =  (SELECT max(id) FROM princess_garment)"
        )
        cursor.execute(
            "UPDATE princess_garment SET arm		= '"
            + str(arm)
            + "'   WHERE id =  (SELECT max(id) FROM princess_garment)"
        )
    if face:
        cursor.execute(
            "UPDATE princess_garment SET face	= '"
            + str(face)
            + "' WHERE id =  (SELECT max(id) FROM princess_garment)"
        )
    if hair:
        cursor.execute(
            "UPDATE princess_garment SET hair	= '"
            + str(hair)
            + "' WHERE id =  (SELECT max(id) FROM princess_garment)"
        )
    if shoes:
        cursor.execute(
            "UPDATE princess_garment SET shoes	= '"
            + str(shoes)
            + "' WHERE id =  (SELECT max(id) FROM princess_garment)"
        )
    if dress:
        cursor.execute(
            "UPDATE princess_garment SET dress	= '"
            + str(dress)
            + "' WHERE id =  (SELECT max(id) FROM princess_garment)"
        )
        cursor.execute(
            "UPDATE princess_garment SET armdress= '"
            + str(armdress)
            + "' WHERE id =  (SELECT max(id) FROM princess_garment)"
        )
    if accessory:
        cursor.execute(
            "UPDATE princess_garment SET accessory= '"
            + str(accessory)
            + "' WHERE id =  (SELECT max(id) FROM princess_garment)"
        )
    cursor.execute(
...
def clean_up(universe):
    universe.db_cursor.execute(
        "UPDATE save SET dirt = "
        + str(universe.level.princesses[0].dirt)
        + " WHERE name = '"
        + universe.level.princesses[0].name
        + "'"
    )
    universe.db.commit()
...
def unlock(universe, type, garment):
    universe.db_cursor.execute(
        "UPDATE unlock SET status = 'unlocked' WHERE type = '"
        + type
        + "' and garment = '"
        + garment
        + "'"
...
def use_message(universe, name):  # , name):
    universe.db_cursor.execute(
        "UPDATE messages SET count = 1 WHERE name = '" + name + "';"
    )
    universe.db.commit()
...
def beat_enemy(universe, enemy):
    universe.db_cursor.execute(
        "UPDATE stage_enemies SET " + enemy + " = 1 WHERE stage = 'Beaten'"
    )
    universe.db.commit()
...
def set_next_ball_clothes(universe, fairy_tale_princess=None, avoid=[], update=False):
    ...
    dresses = [
        i["type"] + "_" + i["garment"] for i in query.unlocked(universe, "dress")
    ]
    accessories = [
        i["type"] + "_" + i["garment"] for i in query.unlocked(universe, "accessory")
    ]
    ...
    for p in princesses_list:
        ...
        if not update:
            row = cursor.execute(
                "SELECT * FROM " + p + " WHERE id = (SELECT MAX(id) FROM " + p + ")"
            ).fetchone()
            cursor.execute(
                "INSERT INTO "
                + p
    ...
    if not update:
        ...
        cursor.execute(
            "INSERT INTO princess_garment VALUES ("
            + str(row["id"] + 1)
            + " , '"
            + str(row["hair_back"])
            + "' , '"
