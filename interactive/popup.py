...
class Message:
    ...
    def __init__(
        ...
        self.pos = (
            (self.universe.width - self.size[0]) / 2,
            self.universe.height - self.size[1],
        )
        self.text_box = self.size[0] * 0.8, self.size[1] * 0.8
        self.image.blit(
            text.in_box(
                settings.third_font,
                settings.third_font_size,
                self.text_box,
                self.image,
                message,
                margin={"left": 150, "top": 40, "right": 150, "bottom": 40},
                alignment="center",
            ),
            self.pos,
        )
...
class Unlocking_Message:
    def __init__(self, universe, unlocking):
        ...
        unlock_message = unlock_template.substitute(
            garment_type=unlocking["type"], name=unlocking["name"]
        )
        self.image.blit(
            text.in_box(
                settings.third_font,
                ...
                self.image,
                unlock_message,
                margin={"left": 150, "top": 60, "right": 150, "bottom": 40},
                alignment="center",
            ),
            self.pos,
        )
...
class Unlocking_Icon:
    ...
    def __init__(self, universe, message, unlocking):
        ...
        self.icon = utils.img.image(
            j(
                directory.princess,
                unlocking["type"] + "_" + unlocking["name"],
                "big_icons",
                "0.png",
            )
        )
