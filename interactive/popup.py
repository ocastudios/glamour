...
class Unlocking_Message():
    ...
    def __init__(self, level,unlocking, font_size = 16):
        ...
        self.pos        = ((self.universe.width - self.size[0])/2, self.universe.height - self.size[1])
        self.text_box   = self.size[0]*.8,self.size[1]*.8
        self.image.blit(text.in_box('FreeSans.ttf',font_size, self.text_box, self.image, "you've unlocked "+unlocking['name'], margin={'left':150,'top':40,'right':150,'bottom':40}, alignment = "center"), self.pos)
    def update_all(self):
        pass
