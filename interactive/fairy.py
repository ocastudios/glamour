...
class Fairy:
    ...
    def __init__(self, pos, universe, margin=p([10, 10, 10, 10]), dirty=False):
        ...
        self.lists_of_images = {
            ...
            "mouth_smile": utils.img.There_and_back_again(
                j(fairy_dir, "fairy_smile"), margin
            ),
            "eyes_eyes": utils.img.There_and_back_again(
                j(fairy_dir, "fairy_eyes"), margin
            ),
            "eyes_blink": utils.img.There_and_back_again(
                j(fairy_dir, "fairy_blink"), margin
            ),
            ...
            "body_fly": utils.img.There_and_back_again(
                j(fairy_dir, "fairy_fly"), margin
            ),
            "body_stand_right": utils.img.There_and_back_again(
                j(fairy_dir, "fairy_stand_right"), margin
            ),
            "body_stand_left": utils.img.There_and_back_again(
                j(fairy_dir, "fairy_stand_left"), margin
            ),
        }
        self.images_strings = [
            "wings_wings",
        ...
        self.parts = [
            self.lists_of_images[i].left[self.lists_of_images[i].number]
            for i in self.images_strings
        ]
    ...
    def select_images(self):
        if self.direction == "left":
            ...
            self.parts = [
                self.lists_of_images[i].right[self.lists_of_images[i].number]
                for i in self.images_strings
            ]
...
class Message:
    ...
    def __init__(
        self,
        universe,
        message="Oops! I just forgot what I had to say... One of us should have a conversation with the programmer.",
    ):
        ...
        self.text_font = pygame.font.Font(
            j(directory.fonts, settings.third_font), settings.fairy_font_size
        )
        self.color = (0, 0, 0, 0)
        self.image.blit(
            utils.text.in_box(
                settings.third_font,
                settings.fairy_font_size,
                self.text_box,
                self.image,
                message,
                {"top": 40, "left": 80, "bottom": 40, "right": 80},
            ),
            self.pos,
        )
        self.button = self.button or widget.Button(
            universe, directory.button_ok, (1200, 820), [0, 0], self.end_message
        )
