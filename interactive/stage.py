...
class Stage:
    ...
    def __init__(self, universe):
        ...
        self.black = Foreground(
            universe, color=(0, 0, 0), path=j(directory.loading, "back.png")
        )
        self.bar = {
            "down": Bar(universe, "down"),
            "up": Bar(universe, "up"),
            "left": Bar(universe, "left"),
            "right": Bar(universe, "right"),
        }
        ...
        self.ball = None
        self.ballroom = {
            "day": scenarios.Background(
                110, universe, j(directory.scenario, "ballroom", "ballroom_day")
            ),
            "night": scenarios.Background(
                110, universe, j(directory.scenario, "ballroom", "ballroom_night")
            ),
        }
        ...
        self.exit_sign = None
        self.loading_icons = (
            utils.img.OneSided(j(directory.loading, "sun_n_moon_shadow")),
    ...
    def what_is_my_height(self, object, pos_x=None):
        try:
            y_height = self.floor_heights[
                (pos_x or int(object.center_distance + round(object.size[0] / 2)))
            ]
        except:
            y_height = self.floor
        return y_height
                ...
                if att == "lights":
                    if self.clock[1].time == "night":
                        for i in self.lights:
                            if i["status"] == "on":
                                screen.blit(
                                    i["images"].list[next(i["images"].itnumber)],
                                    i["position"].pos,
                                )
                else:
        ...
        if self.inside.status == "choosing":
            ...
            if self.inside.__class__ == inside.Inside:
                if self.inside.chosen_item:
                    screen.blit(
                        self.inside.chosen_item.image, self.inside.chosen_item.pos
                    )
            ...
            for i in self.pause.buttons:
                ...
                if princess.image:
                    screen.blit(
                        princess.image,
                        (
                            (u.width / 2) - (princess.image_size[0] / 2),
                            (u.height / 2) - (princess.image_size[1] / 2),
                        ),
                    )
    ...
    def update_insidebar(self):
        if self.inside.status == "inside":
            ...
            if (
                self.bar["down"].pos > self.universe.height
                and self.bar["up"].pos < -self.bar["up"].size[1]
                and self.white.alpha_value == 0
            ):
                pygame.mixer.music.load(self.music)
                pygame.mixer.music.play(-1)
            ...
            if distance < 500 and i == actual_class:
                ...
                if 1 - volume < 0.6:
                    ...
                    if (1 - volume) - present_volume > 0.05:
                        pygame.mixer.music.set_volume(present_volume + 0.05)
                    elif (1 - volume) - present_volume < -0.05:
                        pygame.mixer.music.set_volume(present_volume - 0.05)
                    else:
    ...
    def loading(self):
        if self.black.alpha_value > 100:
            ...
            world_pos = (
                (self.universe.width / 2) - (world.size[0] / 2),
                (self.universe.height / 2) - (world.size[1] / 2),
            )
            carriage_pos = (
                world_pos[0] + p(182, r=False),
                world_pos[1] + p(75, r=False),
            )
            for i in self.loading_icons:
                i.update_number()
            self.universe.screen_surface.blit(
                shadow.list[world.number],
                (world_pos[0], (world_pos[1] + p(60, r=False))),
            )
            self.universe.screen_surface.blit(world.list[world.number], world_pos)
            self.universe.screen_surface.blit(
                carriage.list[carriage.number], carriage_pos
            )
            self.universe.screen_surface.blit(
                self.bar["left"].image, (self.bar["left"].pos, 0)
            )
            self.universe.screen_surface.blit(
                self.bar["right"].image, (self.bar["right"].pos, 0)
            )
            pygame.display.flip()
    ...
    def bring_side_bars(self, darkenning):
        ...
        if self.bar["right"].pos > self.universe.width - (
            self.bar["right"].size[0] - (self.bar["right"].size[0] / 5)
        ):
    ...
    def create_front_scenario(self, street):
        ...
        for i in front_row:
            if i["type"] == "scenario":
                if i["height"] == 0:
                    self.scenarios_front.append(
                        scenarios.Scenario(
                            p(i["xpos"], r=0),
                            j(directory.main, i["directory"]),
                            self.universe,
                        )
                    )
                else:
    ...
    def create_stage(self, translatable_name, goalpos, hardname):
        ...
        self.panel = [
            widget.GameText(self.universe, t(translatable_name), (300, 20)),
            None,
            glamour_stars.Glamour_Stars(self.universe),
            widget.GameText(
                self.universe,
                self.princesses[0].name,
                (660, 47),
                main_font=settings.second_font,
            ),
    ...
    def BathhouseSt(self, goalpos=None, clean_princess=False):
        ...
        gates = (
            [
                (1063, 453),
                "bathhouse/door/",
                inside.Inside(self.universe, "shower", []),
                True,
            ],
            [(5206, 500), "home/door/", inside.Home(self.universe), False],
            [
                (9305, 503),
                "magic_beauty_salon/door/",
                inside.Inside(
                    self.universe,
                    "hair",
                    database.query.unlocked(self.universe, "hair", "garment", 4),
                ),
                False,
            ],
        )
        self.loading()
        doors = (
            [bathhousegate[0], self.ShoesSt, shoegate[2]],
            [bathhousegate[1], self.AccessorySt, accessorygate[0]],
        )
        self.loading()
        self.gates = [
            scenarios.BuildingDoor(
                p(i[0]), j(self.directory, i[1]), self.universe, i[2], bath=i[3]
            )
            for i in gates
        ]
        self.loading()
        self.gates.extend(
            [scenarios.Gate(i[0], self.universe, i[1], goalpos=i[2]) for i in doors]
        )
        self.loading()
        self.select_enemies(
            ("schnauzer", "butterfly", "old_lady", "footboy", "bird"), "BathhouseSt"
        )
        self.floor_image = [
            floors.Floor(c, j(self.directory, "floor", "tile"), self.universe)
            for c in range(24)
        ]
        self.loading()
    ...
    def DressSt(self, goalpos=None):
        ...
        self.animated_scenarios = [
            scenarios.Scenario(
                0, j(self.directory, "Dress_Tower", "flag"), self.universe
            )
        ]
        self.loading()
        self.select_enemies(
            ("schnauzer", "butterfly", "old_lady", "footboy", "bird", "carriage"),
            "DressSt",
        )
        self.gates = [
            scenarios.BuildingDoor(
                p((155, 318)),
                j(self.directory, "Dress_Tower", "door"),
                self.universe,
                inside.Inside(
                    self.universe,
                    "dress",
                    database.query.unlocked(self.universe, "dress", "garment", 4),
                ),
            ),
            scenarios.BuildingDoor(
                p((9180, 430)),
                j(self.directory, "snow_white_castle", "door"),
                self.universe,
                inside.Princess_Home(self.universe, settings.Snow_White),
            ),
            scenarios.Gate(
                dressgate[0], self.universe, self.AccessorySt, goalpos=accessorygate[2]
            ),
            scenarios.Gate(
                dressgate[1], self.universe, self.ShoesSt, goalpos=shoegate[1]
            ),
            scenarios.Gate(
                dressgate[2], self.universe, self.MakeupSt, goalpos=makeupgate[0]
            ),
        ]
        self.loading()
        self.floor_image = [
            floors.Floor(c, j(self.directory, "floor"), self.universe)
            for c in range(30)
        ]
        self.loading()
    ...
    def AccessorySt(self, goalpos=None):
        ...
        self.select_enemies(
            ("schnauzer", "butterfly", "old_lady", "bird"), "AccessorySt"
        )
        ...
        self.gates = [
            scenarios.BuildingDoor(
                p((330, 428)),
                j(self.directory, "accessory_tower", "door"),
                self.universe,
                inside.Inside(
                    self.universe,
                    "accessory",
                    database.query.unlocked(self.universe, "accessory", "garment", 4),
                ),
            ),
            scenarios.BuildingDoor(
                p((8809, 425)),
                j(self.directory, "castle", "door"),
                self.universe,
                inside.Princess_Home(self.universe, settings.Cinderella),
            ),
            scenarios.Gate(
                accessorygate[0],
                self.universe,
                self.BathhouseSt,
                goalpos=bathhousegate[1],
            ),
            scenarios.Gate(
                accessorygate[1], self.universe, self.MakeupSt, goalpos=makeupgate[1]
            ),
            scenarios.Gate(
                accessorygate[2], self.universe, self.DressSt, goalpos=dressgate[0]
            ),
        ]
        self.loading()
        self.floor_image = [
            floors.Floor(fl, j(self.directory, "floor", "tile"), self.universe)
            for fl in range(30)
        ]
        self.loading()
        self.floor_image.extend(
            [
                floors.Water(wat, j(self.directory, "water", "tile"), self.universe)
                for wat in range(11)
            ]
        )
        self.loading()
        ...
        self.floor_image.extend(
            [
                floors.Water2(wat, j(self.directory, "water", "tile"), self.universe)
                for wat in range(11)
            ]
        )
        self.loading()
        [
            floors.Drain(
                j(self.directory, "floor", i[0] + "_bank_front"), i[1], self.universe
            )
            for i in [("left", 2), ("right", 3), ("left", 20), ("right", 21)]
        ]
        self.loading()
    ...
    def MakeupSt(self, goalpos=None):
        ...
        self.select_enemies(
            ("carriage", "schnauzer", "butterfly", "old_lady", "footboy", "bird"),
            "MakeupSt",
        )
        self.gates.extend(
            [
                scenarios.Gate(makeupgate[x], self.universe, y, goalpos=z)
                for x, y, z in (
        ...
        self.gates.extend(
            [
                scenarios.BuildingDoor(
                    p((130, 225)),
                    j(self.directory, "make-up_castle", "door"),
                    self.universe,
                    inside.Inside(
                        self.universe,
                        "face",
                        database.query.unlocked(self.universe, "face", "garment", 4),
                    ),
                ),
                scenarios.BuildingDoor(
                    p((9082, 301)),
                    j(self.directory, "sleeping_castle", "door"),
                    self.universe,
                    inside.Princess_Home(self.universe, settings.Sleeping_Beauty),
                ),
            ]
        )
        self.loading()
        self.floor_image = [
            floors.Floor(fl, j(self.directory, "floor"), self.universe)
            for fl in range(30)
        ]
        self.loading()
        available_animals = random.sample(
            (enemy.Lion, enemy.Monkey, enemy.Elephant, enemy.Penguin, enemy.Giraffe), 3
        )
        self.animated_scenarios = [i(self.universe) for i in available_animals]
        self.loading()
        self.animated_scenarios.append(
            scenarios.Scenario(
                p(2923, r=0), j(self.directory, "zoo", "base"), self.universe
            )
        )
        self.loading()
        tail = None
    ...
    def ShoesSt(self, goalpos=None):
        ...
        self.animated_scenarios = [
            scenarios.Scenario(
                p(7137, r=0), j(self.directory, "fountain", "base"), self.universe
            )
        ]
        self.loading()
        self.select_enemies(
        ...
        self.gates = [
            scenarios.BuildingDoor(
                p((372, 273)),
                j(self.directory, "shoes_tower", "door"),
                self.universe,
                inside.Inside(
                    self.universe,
                    "shoes",
                    database.query.unlocked(self.universe, "shoes", "garment", 4),
                ),
            ),
            scenarios.BuildingDoor(
                p((9440, 374)),
                j(self.directory, "rapunzel_castle", "door"),
                self.universe,
                inside.Princess_Home(self.universe, settings.Rapunzel),
            ),
            scenarios.Gate(
                shoegate[0], self.universe, self.MakeupSt, goalpos=makeupgate[2]
            ),
            scenarios.Gate(
                shoegate[1], self.universe, self.DressSt, goalpos=dressgate[1]
            ),
            scenarios.Gate(
                shoegate[2], self.universe, self.BathhouseSt, goalpos=bathhousegate[0]
            ),
        ]
        self.floor_image = [
            floors.Floor(c, j(self.directory, "floor"), self.universe)
        ...
        if up_or_down in ("left", "right"):
            if up_or_down == "left":
                ...
                self.image = utils.img.image(
                    j(directory.left_bar, "0.png"), invert=True
                )
                self.size = self.image.get_size()
            ...
            image_prep = pygame.Surface(
                (screen_size[0], tile_size[1]), pygame.SRCALPHA
            ).convert_alpha()
    ...
    def __init__(self, universe, street):
        ...
        if street + str(p(1440, 900)) in cached_images:
            self.image = pygame.image.load(
                os.path.join(
                    directory.cache, "images", street + str(p([1440, 900])) + ".png"
                )
            ).convert_alpha()
        else:
            self.universe.stage.scenarios_prep = []
            self.image = pygame.Surface(
                (p(9600, r=0), universe.height), pygame.SRCALPHA
            ).convert_alpha()
            for i in scenario_row:
                if i["invert"] == 1:
                    if i["height"] != 0:
                        img = scenarios.Scenario(
                            p(i["xpos"], r=False),
                            j(directory.main, i["directory"]),
                            self.universe,
                            invert=True,
...
class Pause:
    def __init__(self, universe):
        self.status = "outside"
        self.universe = universe
        resume = widget.GameText(
            self.universe, t("Resume"), (360, 400), main_font=settings.second_font
        )
        ok_pos = (
            d(resume.pos[0] + (resume.size[0] / 2)),
            d(resume.pos[1] + (resume.size[1])) + 50,
        )
        ok_button = widget.Button(
            self.universe, directory.button_ok, ok_pos, [0, 0], self.resume
        )
        leave = widget.GameText(
            self.universe, t("Quit"), (1080, 400), main_font=settings.second_font
        )
        cancel_pos = (
            d(leave.pos[0] + (leave.size[0] / 2)),
            d(leave.pos[1] + (leave.size[1])) + 50,
        )
        cancel_button = widget.Button(
            self.universe, directory.button_cancel, cancel_pos, [0, 0], self.exit_game
        )
        title = widget.GameText(
            self.universe, t("Game Paused"), (720, 100), main_font=settings.second_font
        )
        check_closet = widget.Button(
            self.universe, t("Check your closet"), (720, 700), [0, 0], self.set_closet
        )
        ...
        self.menu = [
            (i.pos[0] + (i.size[0] / 4), i.pos[1] + (i.size[1] / 4))
        ...
        self.icons = {
            "accessory_beret": [
                p((240, 82)),
                t("The beret accessory, earnd for jumping with the penguin."),
            ],
            "accessory_crown": [
                p((350, 82)),
                t("The crown accessory, unlocked from the start."),
            ],
            "accessory_shades": [
                p((460, 82)),
                t("The shades accessory, earned after trampling 20 chicks."),
            ],
            "accessory_mask": [
                p((570, 82)),
                t("The mask, earned for debuting three hairstyles in balls."),
            ],
            "accessory_purse": [
                p((680, 82)),
                t("The purse accessory, unlocked from the start."),
            ],
            "accessory_ribbon": [
                p((790, 82)),
                t("The ribbon accessory, unlocked from the start."),
            ],
            "dress_indian": [
                p((900, 82)),
                t("The indian dress, earned for dressing as a geisha."),
            ],
            "dress_kimono": [
                p((1010, 82)),
                t("The kimono, earned for being crossed by the carriage."),
            ],
            "dress_pink": [
                p((1120, 82)),
                t("The pink dress, unlocked from the start."),
            ],
            "dress_red": [p((240, 192)), t("The red dress, unlocked from the start.")],
            "dress_plain": [
                p((350, 192)),
                t("The simple dress, unlocked from the start."),
            ],
            "dress_yellow": [
                p((460, 192)),
                t("The yellow dress, earned for beating all kinds of enemies."),
            ],
            "face_eyelids": [
                p((570, 192)),
                t("The eyelids make-up, unlocked from the start."),
            ],
            "face_eyeshades": [
                p((680, 192)),
                t("The eyeshades make-up, unlocked from the start."),
            ],
            "face_geisha": [
                p((790, 192)),
                t("The geisha look, earned for bathing while clean."),
            ],
            "face_indian": [
                p((900, 192)),
                t("The indian look, earned for kissing the lion."),
            ],
            "face_lipstick": [
                p((1010, 192)),
                t("The lipstick make-up, earned for entering your own castle."),
            ],
            "face_simple": [
                p((1120, 192)),
                t("The simple make-up, unlocked from the start."),
            ],
            "hair_black": [
                p((240, 302)),
                t("The black hairstyle, unlocked from the start."),
            ],
            "hair_braid_and_tail": [
                p((350, 302)),
                t("The braid and tail hairstyle, unlocked from the start."),
            ],
            "hair_brown": [
                p((460, 302)),
                t("The brown hairstyle, unlocked from the start."),
            ],
            "hair_cinderella": [
                p((570, 302)),
                t("Cinderella's hairstyle, earned for visiting her."),
            ],
            "hair_geisha": [
                p((680, 302)),
                t("The geisha hairstyle, unlocked from the start."),
            ],
            "hair_rapunzel": [
                p((790, 302)),
                t("Rapunzel's hairstyle, earned for visiting her."),
            ],
            "hair_rastafari": [
                p((900, 302)),
                t("The rastafari hairstyle, unlocked from the start."),
            ],
            "hair_red": [
                p((1010, 302)),
                t("The red hairstyle, unlocked from the start."),
            ],
            "hair_short": [
                p((1120, 302)),
                t("The short hairstyle, unlocked from the start."),
            ],
            "hair_sleeping": [
                p((240, 412)),
                t("Sleeping Beauty's hairstyle, earned for visiting her."),
            ],
            "hair_snowwhite": [
                p((350, 412)),
                t("Snow White's hairstyle, earned for visiting her."),
            ],
            "hair_yellow": [
                p((460, 412)),
                t("The blonde hairstyle, unlocked from the start."),
            ],
            "shoes_boots": [
                p((570, 412)),
                t("The go-go boots, earned by crossing the drains clean."),
            ],
            "shoes_crystal": [
                p((680, 412)),
                t("The glass slippers, earned for wearing the dream outfit."),
            ],
            "shoes_flower": [
                p((790, 412)),
                t("The flower sandals, earned for visiting all streets in one day."),
            ],
            "shoes_red": [p((900, 412)), t("The red shoes, unlocked from the start.")],
            "shoes_slipper": [
                p((1010, 412)),
                t("The slippers, unlocked from the start."),
            ],
            "shoes_white": [
                p((1120, 412)),
                t("The white shoes, unlocked from the start."),
            ],
        }
        ...
        for i in self.icons:
            if i not in unlocked_names:
                self.unlocked_items.append(
                    Closet_Icon(self.universe, None, None, self.icons[i], locked=True)
                )
        self.unlocked_items.extend(
            [Dearhearts(self.universe.stage.princesses[0].points)]
        )
        self.close_closet = widget.Button(
            self.universe,
            directory.button_cancel,
            (1300, 800),
            [0, 0],
            self.clear_closet,
        )
    ...
    def exit_game(self):
        ...
        self.universe.menu.vertical_bar["position"] = -self.universe.menu.vertical_bar[
            "size"
        ][0]
        self.universe.menu.vertical_bar["side"] = "left"
        self.universe.menu.vertical_bar["call_bar"] = "left"
        self.universe.menu.vertical_bar["image"] = pygame.transform.flip(
            self.universe.menu.vertical_bar["image"], 1, 0
        )
...
class Closet_Icon:
    def __init__(self, universe, garment_type, garment, icon, locked=None):
        ...
        if self.locked:
            ...
            self.medium = utils.img.image(
                os.path.join(directory.interface, "closet", "blocked.png")
            )
            self.big = None
        else:
            ...
            text = icon[1]
            self.medium = utils.img.image(
                os.path.join(directory.princess, self.name, "medium", "0.png")
            )
            self.big = utils.img.image(
                os.path.join(directory.princess, self.name, "big_icons", "0.png")
            )
    ...
    def __init__(self, points):
        ...
        for i in boyfriends_ranking:
            if points >= i[2]:
                self.image.blit(
                    utils.img.image(
                        os.path.join(directory.boyfriends, i[0], "medium", "0.png")
                    ),
                    p(i[1]),
                )
