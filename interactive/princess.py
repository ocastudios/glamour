...
class Princess:
    ...
    def __init__(self, universe, INSIDE=False, xpos=None):
        ...
        self.pos = [
            int(universe.center_x) + self.center_distance,
            universe.floor - universe.level.what_is_my_height(self) - self.size[1],
        ]
        print("	creating images:")
        print("		princess images")
        for act in ["walk", "stay", "kiss", "fall", "jump", "ouch", "celebrate"]:
            self.__dict__[act + "_img"] = utils.img.MultiPart(
                self.ordered_directory_list(act)
            )
        self.run_away_img = utils.img.Ad_hoc(
            self.walk_img.left[::2], self.walk_img.right[::2]
        )
        print("		dirt images")
        self.dirties = [
            Dirt(self.universe, directory.princess + "/" + d, self.pos)
            for d in ("dirt1", "dirt2", "dirt3")
        ]
        self.images = None
        ...
        print("		jump sounds")
        self.jumpsound = pygame.mixer.Sound(
            os.path.join(directory.princess_sounds, "pulo.ogg")
        )
    ...
    def ordered_directory_list(self, action):
        ...
        row = cursor.execute(
            "SELECT * FROM princess_garment WHERE id=(SELECT MAX(id) FROM princess_garment)"
        ).fetchone()
        for part in [
            "hair_back",
            "skin",
    ...
    def dirt_cloud_funciton(self):
        if 0 < self.status["hurt"] < 24:
            if self.status["hurt"] > len(self.dirt_cloud.left):
                dirt_cloud_image = self.dirt_cloud.left[
                    self.status["hurt"] - 1 - len(self.dirt_cloud.left)
                ]
            else:
            ...
            if not self.status["hurt"]:
                for e in self.universe.level.enemies:
                    if e.__class__ in (
                        ...
                        enemy.BroomingDust,
                        enemy.Banana,
                    ) and self.rect.colliderect(e.rect):
                        self.get_dirty()
                    ...
                    if e.__class__ == enemy.Butterfly:
                        if (
                            ...
                            and self.status["excited"] == 0
                        ):
                            self.status["excited"] += 1
                if self.universe.level.viking_ship and self.rect.colliderect(
                    self.universe.level.viking_ship.talk_balloon_rect
                ):
                    self.get_dirty()
                if self.universe.level.name == "accessory":
                    if (
                        self.pos[1] + self.size[1] - p(20)
                        > self.universe.level.water_level
                    ):
                        self.get_dirty()
            else:
    ...
    def get_dirty(self):
        if self.dirt <= 2:
            self.status["hurt"] += 1
            self.dirt += 1
            self.universe.db_cursor.execute(
                "UPDATE save SET dirt = "
                + str(self.dirt)
                + " WHERE name = '"
                + self.name
                + "'"
    ...
    def throwkiss(self):
        ...
        D = {
            "right": {"rect_correction": 0, "effect_correction": 0},
            "left": {
                "rect_correction": p(200) - ((self.kiss["count"]) * p(44)),
                "effect_correction": p(200),
            },
        }
        ...
        self.effects.append(
            Effect(
                kissimage,
                (
                    self.pos[0] - D[self.kiss["direction"]]["effect_correction"],
                    self.kiss["height"],
                ),
            )
        )
        self.kiss["rect"] = pygame.Rect(
            (
                self.pos[0] + D[self.kiss["direction"]]["rect_correction"],
                self.kiss["height"],
            ),
