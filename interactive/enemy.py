...
class Schnauzer:
    ...
    def __init__(self, pos, universe, margin=p([20, 20, 20, 20])):
        self.music = {
            "sound": pygame.mixer.Sound(os.path.join(directory.music, "schnauzer.ogg")),
            "weight": 5,
        }
        self.bow = pygame.mixer.Sound(
            os.path.join(directory.enemies_sounds, "Schnauzer_bark.ogg")
        )
        self.center_distance = pos
        for i in ["kissed", "walk", "stay"]:
            self.__dict__[i] = utils.img.TwoSided(
                os.path.join(directory.enemies, "Schnauzer", i)
            )
        ...
        self.rect = pygame.Rect(
            ((self.pos[0] + int(self.size[0] / 2.8)), self.pos[1]),
            (self.size[0] - int(self.size[0] / 2.8), self.size[1]),
        )
    ...
    def set_pos(self, cross=30):
        ...
        if self.move:
            ...
            new_y = get_height(
                ...
                pos_x=int(
                    ...
                    + (self.speed * towards[self.direction])
                    + modifier
                ),
            )
        ...
        self.pos = [
            self.universe.center_x + self.center_distance,
            self.floor + self.margin[2] - (self.size[1]),
        ]
    ...
    def set_image(self):
        if self.gotkissed >= 1:
            ...
            if self.move:
                ...
                self.image = (
                    images.right[0] if self.lookside % 2 == 0 else images.left[0]
                )
...
class Carriage:
    ...
    def __init__(self, pos, universe, margin=p([10, 10, 10, 10])):
        ...
        for i in ["walk", "stay"]:
            self.__dict__[i] = utils.img.TwoSided(
                os.path.join(directory.enemies, "Carriage", i), margin
            )
        self.image = self.walk.left[0]
        ...
        self.direction = random.choice(["left", "right"])
        self.rect = pygame.Rect(
            (self.pos[0] + self.correction, (universe.level.floor - self.pos[1])),
            (self.size),
        )
        self.gotkissed = 0
        self.image_number = 0
    ...
    def set_pos(self):
        ...
        if self.move:
            if self.direction == "right":
                ...
                self.center_distance -= self.speed
                self.rect = pygame.Rect(
                    (self.pos[0] + self.correction, self.pos[1]), (self.size)
                )
...
class Butterfly:
    ...
    def __init__(self, pos, universe):
        self.music = self.music or {
            "sound": pygame.mixer.Sound(os.path.join(directory.music, "butterfly.ogg")),
            "weight": 1,
            "playing": False,
        }
        self.walk = self.walk or utils.img.TwoSided(
            os.path.join(directory.enemies, "Butterfly", "walk")
        )
        self.walk_number = 0
        ...
        self.rect = pygame.Rect(
            (
                ...
                (universe.level.floor - self.pos[1] + (self.size[1] / 4)),
            ),
            (self.size),
        )
        self.gotkissed = 0
    ...
    def set_pos(self):
        ...
        if self.direction == "right":
            ...
            self.center_distance -= self.speed
        self.rect = pygame.Rect(
            ((self.pos[0] + (self.size[0] / 4)), self.height + (self.size[1] / 4)),
            (self.size),
        )
...
class OldLady:
    ...
    def __init__(self, pos, universe, margin=p([10, 10, 10, 10])):
        self.music = {
            "sound": pygame.mixer.Sound(
                os.path.join(directory.enemies_sounds, "old_lady.ogg")
            ),
            "weight": 3,
        }
        self.center_distance = pos
        self.walk = utils.img.TwoSided(
            os.path.join(directory.enemies, "OldLady", "walk"), margin
        )
        self.wave = utils.img.There_and_back_again(
            os.path.join(directory.enemies, "OldLady", "hover"), margin
        )
        self.broom = utils.img.There_and_back_again(
            os.path.join(directory.enemies, "OldLady", "act"), margin
        )
        self.kissed = utils.img.There_and_back_again(
            os.path.join(directory.enemies, "OldLady", "kissed"), margin
        )
        self.image = self.walk.left[0]
        ...
        self.rect = pygame.Rect(
            (
                ...
                (self.universe.level.floor - self.pos[1]),
            ),
            (self.size),
        )
        self.image_number = 0
    ...
    def got_kissed(self):
        ...
        if (
            self.action not in ("wave", "kissed")
            and princess.kiss["rect"]
    ...
    def set_pos(self):
        ...
        if self.action == "walk":
            if self.direction == "right":
                ...
                if (self.universe.floor - next_height) <= (
                    self.floor - self.size[1]
                ) - p(5):
                    self.center_distance += self.speed
            else:
                self.center_distance -= self.speed
        ...
        self.rect = pygame.Rect(
            (
                (self.pos[0] + (self.size[0] / 2)),
                (self.universe.level.floor - self.size[1]),
            ),
            (self.size),
        )
...
class BroomingDust:
    ...
    def __init__(self, lady):
        ...
        self.images = utils.img.TwoSided(
            os.path.join(directory.enemies, "OldLady", "dirt")
        )
        ...
        rect_pos = (
            self.pos[0] + self.rect_list[self.images.number]["pos"][0],
    ...
    def update_all(self):
        if (self.lady.action == "broom" and self.lady.image_number == 1) or (
            ...
            if self.direction == "left":
                ...
                rect_pos = (
                    self.pos[0] + p(144) - self.rect_list[self.images.number]["pos"][0],
                    self.pos[1] + self.rect_list[self.images.number]["pos"][1],
                )
            rect_rect = self.rect_list[self.images.number]["size"]
            self.rect = pygame.Rect(rect_pos, rect_rect)
...
class Lion:
    ...
    def __init__(self, universe):
        ...
        self.base = utils.img.There_and_back_again(
            os.path.join(directory.enemies, "Lion", "base"), exclude_border=True
        )
        self.growl = utils.img.There_and_back_again(
            os.path.join(directory.enemies, "Lion", "growl"), exclude_border=True
        )
        self.kissed = utils.img.TwoSided(
            os.path.join(directory.enemies, "Lion", "kissed")
        )
...
class Tail:
    ...
    def __init__(self, lion):
        ...
        self.images = utils.img.There_and_back_again(
            os.path.join(directory.enemies, "Lion", "tail")
        )
...
class Elephant:
    ...
    def __init__(self, universe):
        ...
        for i in ["base", "hover"]:
            self.__dict__[i] = utils.img.There_and_back_again(
                os.path.join(directory.enemies, "Elephant", i), exclude_border=True
            )
        self.image = self.base.left[0]
        ...
        self.pos = [
            self.universe.center_x + self.center_distance,
            (self.universe.level.floor - self.image.get_height()) + 15,
        ]
        self.direction = random.choice(["left", "right"])
        self.gotkissed = 0
        self.image_number = 0
...
class Giraffe:
    ...
    def __init__(self, universe):
        ...
        ordered_directory_list = (
            os.path.join(directory.enemies, "giraffe", "base"),
            os.path.join(directory.enemies, "giraffe", "chew"),
        )
        self.chewing_images = utils.img.MultiPart(ordered_directory_list)
        self.hover_images = utils.img.TwoSided(
            os.path.join(directory.enemies, "giraffe", "hover")
        )
        self.image = self.chewing_images.left[0]
        self.universe = universe
        self.pos = [
            universe.center_x + self.center_distance,
            (self.universe.level.floor - self.image.get_height()),
        ]
        self.direction = random.choice(["left", "right"])
        self.image_number = 0
        self.count = 0
        ...
        if -p(400) < self.pos[0] < p(1490):
            if self.count == 2:
                self.image = self.chewing_images.left[
                    next(self.chewing_images.itnumber)
                ]
                self.count = 0
            self.count += 1
        self.pos[0] = self.universe.center_x + self.center_distance
...
class Penguin:
    ...
    def __init__(self, universe):
        self.music = None
        self.images = utils.img.There_and_back_again(
            os.path.join(directory.enemies, "penguin", "jump"), exclude_border=True
        )
        ...
        self.pos = [
            self.universe.center_x + self.center_distance,
            (self.universe.level.floor - self.image.get_height()) + p(15),
        ]
        self.direction = "left"
        self.gotkissed = 0
        self.image_number = 0
    ...
    def update_all(self):
        ...
        if -p(400) < self.pos[0] < p(1490):
            self.update_image()
            self.jumping()
            self.floor = self.universe.floor - self.universe.level.what_is_my_height(
                self
            )
            if self.action == "dance":
                self.dancing()
...
class Monkey:
    ...
    def __init__(self, universe):
        ...
        for i in ["stay", "hover", "happy", "throw", "attack"]:
            self.__dict__[i] = utils.img.TwoSided(
                os.path.join(directory.enemies, "Monkey", i)
            )
        self.direction = random.choice(["right", "right"])
...
class Banana:
    ...
    def __init__(self, universe, monkey):
        ...
        self.images = {
            "throwing": utils.img.TwoSided(
                os.path.join(directory.enemies, "Monkey", "banana", "throwing")
            ),
            "quiet": utils.img.TwoSided(
                os.path.join(directory.enemies, "Monkey", "banana", "quiet")
            ),
            "thrown": utils.img.TwoSided(
                os.path.join(directory.enemies, "Monkey", "banana", "thrown")
            ),
        }
        ...
        self.rect = pygame.Rect(
            (
                ...
                self.pos[1] + self.pos_correction[1],
            ),
            self.banana_size,
        )
    ...
    def update_all(self):
        ...
        if self.status == "thrown":
            ...
            if self.speed["actual"] > 0:
                self.speed["actual"] = round(
                    self.speed["actual"] - self.speed["resistance"]
                )
            else:
                self.speed["actual"] = 0
...
class VikingShip:
    ...
    def __init__(self, universe):
        self.music = self.music or {
            "sound": pygame.mixer.Sound(
                os.path.join(directory.enemies_sounds, "viking_ship.ogg")
            ),
            "weight": 5,
        }
        self.universe = universe
        self.base = utils.img.TwoSided(
            os.path.join(directory.enemies, "VikingShip", "base")
        )
        sailor_body = utils.img.image(
            os.path.join(
                directory.enemies, "VikingShip", "viking_sailor", "body", "0.png"
            )
        )
        ...
        self.pos = [
            universe.center_x + self.center_distance,
            universe.level.floor - self.image_height + p(200) + next(self.height),
        ]
        self.gotkissed = 0
        self.image_number = 0
        self.speed = p(3)
        ...
        self.head_list = {
            "normal": VikingPart(
                self,
                os.path.join("viking_sailor", "head_normal"),
                pos_x=D[self.direction]["head"][0],
                pos_y=D[self.direction]["head"][1],
            ),
            "hover": VikingPart(
                self,
                os.path.join("viking_sailor", "head_hover"),
                pos_x=D[self.direction]["head"][0],
                pos_y=D[self.direction]["head"][1],
            ),
            "angry": VikingPart(
                self,
                os.path.join("viking_sailor", "head_angry"),
                pos_x=D[self.direction]["head"][0],
                pos_y=D[self.direction]["head"][1],
            ),
            "talk": VikingPart(
                self,
                os.path.join("viking_sailor", "head_talk"),
                pos_x=D[self.direction]["head"][0],
                pos_y=D[self.direction]["head"][1],
            ),
        }
        ...
        self.balloon = Balloon(
            self,
            brand="talk",
            pos_x=self.D[self.direction]["talk"][0],
            pos_y=self.D[self.direction]["talk"][1],
        )
    ...
    def update_all(self):
        ...
        if -p(1000) < self.pos[0] < p(1800):  # check if Viking is on screen
            if self.wave not in self.universe.level.floor_image:
                self.universe.level.floor_image.extend(
                    [self.flag, self.wave, self.head]
                )
            else:
                self.head = self.head_list[
                    self.mood
                ]  # change faces acording to the mood
            ...
            if self.mood == "normal":
                ...
                if self.count > 60:
                    ...
                    self.count = 0
                    self.reset_talk()
                self.talk_balloon_rect = pygame.Rect(
                    self.balloon.pos, self.balloon.size
                )
    ...
    def moving(self):
        ...
        self.pos[0] = self.universe.center_x + self.center_distance
        self.pos[1] = (
            self.universe.level.floor - self.image_height + p(200) + next(self.height)
        )
        self.sailor_rect = pygame.Rect(self.head.pos, self.head.size)
        self.flag.pos = (
            self.pos[0] + (self.flag.pos_x - self.flag.size[0]),
            self.pos[1] + self.flag.pos_y,
        )
        self.wave.pos = self.pos[0] + (
            self.wave.pos_x - self.wave.size[0]
        ), self.universe.level.floor_image[-5].pos[1] - (self.wave.size[1] - p(20))
    ...
    def check_mood_talking(self):
        if self.count > 100 and random.randint(0, 20) == 0:
            ...
            self.balloon.pos = (
                self.pos[0] + self.balloon.pos_x,
                self.pos[1] + self.balloon.pos_y,
            )
            self.count = 0
    ...
    def check_mood_angry(self):
        D = self.D
        if self.universe.level.princesses[0].kiss and self.sailor_rect.colliderect(
            self.universe.level.princesses[0].kiss["rect"]
        ):
...
class VikingPart:
    ...
    def __init__(self, ship, part, pos_x=0, pos_y=0):
        ...
        self.images = utils.img.TwoSided(
            os.path.join(directory.enemies, "VikingShip", part)
        )
...
class Balloon:
    talk_image_tpl = utils.img.image(
        os.path.join(directory.enemies, "VikingShip", "talk_balloon", "0.png")
    )
    shout_image_tpl = utils.img.image(
        os.path.join(directory.enemies, "VikingShip", "shout_balloon", "0.png")
    )
    curses = [
        os.path.join(directory.enemies, "VikingShip", "curses", str(index) + ".png")
...
class FootBoy:
    ...
    def __init__(self, pos, universe):
        self.music = self.music or {
            "sound": pygame.mixer.Sound(
                os.path.join(directory.enemies_sounds, "fabrizio.ogg")
            ),
            "weight": 4,
            "playing": False,
        }
        ...
        self.running = utils.img.There_and_back_again(
            walk1,
            second_dir=walk2,
            extra_part=os.path.join(
                directory.enemies, "FootBoy", "happy_face", "first_cycle"
            ),
            second_extra_part=os.path.join(
                directory.enemies, "FootBoy", "happy_face", "second_cycle"
            ),
        )
        self.running_mad = utils.img.There_and_back_again(
            ...
            second_dir=walk2,
            extra_part=os.path.join(
                directory.enemies, "FootBoy", "mad_face", "first_cycle"
            ),
            second_extra_part=os.path.join(
                directory.enemies, "FootBoy", "mad_face", "second_cycle"
            ),
        )
        self.running_kissed = utils.img.There_and_back_again(
            os.path.join(directory.enemies, "FootBoy", "kissed")
        )
        self.standing_body = utils.img.TwoSided(
            os.path.join(directory.enemies, "FootBoy", "walk_body", "stand")
        )
        ...
        self.universe.level.enemies.append(
            FootBall(random.randint(int(p(4000)), int(p(6000))), self)
        )
...
class FootBall:
    ...
    def __init__(self, center_distance, footboy):
        ...
        self.images = utils.img.TwoSided(
            os.path.join(directory.enemies, "FootBoy", "ball")
        )
        self.image = self.images.left[0]
        self.size = (
            self.images.left[0].get_width(),
            self.images.left[0].get_height() - p(7),
        )
        self.real_size = self.size
        self.pos = [
            self.footboy.universe.center_x + self.center_distance,
            self.footboy.universe.level.floor - self.size[1],
        ]
        self.speed = 0
        self.rect = pygame.Rect(self.pos, self.size)
    ...
    def update_all(self):
        ...
        if self.footboy.body != self.footboy.running:
            ...
            if (
                self.footboy.direction == "left"
                and self.pos[0] - p(400) > self.footboy.pos[0]
            ):
...
class Bird:
    ...
    flying = utils.img.There_and_back_again(
        os.path.join(directory.enemies, "Birdy", "fly")
    )
    walking = utils.img.There_and_back_again(
        os.path.join(directory.enemies, "Birdy", "walk")
    )
    ...
    def __init__(self, pos, universe):
        ...
        self.pos = [
            universe.center_x + self.center_distance,
            universe.level.floor + self.margin[2] - self.size[1] + p(10),
        ]
        self.direction = "left"
        self.counter = 0
        self.image_number = 0
        ...
        for i in self.universe.level.enemies:
            if i.__class__ == Hawk:
                hawks += 1
        if not hawks:
            universe.level.enemies.append(
                Hawk((universe.width + int(p(600)), int(-p(300))), universe, self)
            )
            self.original = True
        else:
            self.original = False
    ...
    def update_all(self, cross=30):
        ...
        if self.counter >= random.randint(50, 500):
            ...
            self.counter = 0
        towards = {"right": 1, "left": -1}
        obstacle = level.floor - level.what_is_my_height(
            self, self.center_distance + (round(self.size[0]) * towards[self.direction])
        )
        new_y = level.what_is_my_height(
            self,
            pos_x=int(self.center_distance + (self.speed * towards[self.direction]))
            + int(round(self.size[0] / 2)),
        )
        ...
        if self.disturbed["count"] == 3:
            ...
            self.disturbed["spot"] = (
                self.pos[0],
                (self.universe.floor - (level.princesses[0].size[1] / 2)),
            )
...
class Hawk:
    ...
    def __init__(self, pos, universe, bird):
        self.music = self.music or {
            "sound": pygame.mixer.Sound(os.path.join(directory.music, "hawk.ogg")),
            "weight": 2,
            "playing": False,
        }
        self.bird = bird
        self.center_distance = pos[0]
        self.flying = utils.img.There_and_back_again(
            os.path.join(directory.enemies, "Hawk", "fly")
        )
        self.attacking = utils.img.TwoSided(
            os.path.join(directory.enemies, "Hawk", "attack")
        )
    ...
    def update_all(self):
        ...
        if self.mood == "angry":
            ...
            if self.pos[1] > self.bird.pos[1] - (
                self.universe.level.princesses[0].size[1] / 2
            ):  # self.level.universe.floor - int(round(350*scale)):
            ...
            if -self.size[1] < self.pos[1]:
                ...
                self.mood = "calm"
        if self.mood == "calm":
            self.center_distance += self.speed * 0.5
