...
def image(dir, invert = False, alpha = True, pre_rendered = False):
...
def find_images(dir):
    ...
    if not complete_path:
        dir = main_dir+'/'+dir
    try:
        rendered_dir = dir+'scale'+str(int(round(1440*scale)))+'x'+str(int(round(900*scale)))
        os.listdir(rendered_dir)
        return [image(rendered_dir+'/'+item, pre_rendered=True) for item in sorted(os.listdir(rendered_dir)) if ( item[-4:] == '.png' or item[-4:]== '.PNG')]
    except:
        newdir = dir+'scale'+str(int(round(1440*scale)))+'x'+str(int(round(900*scale)))+'/'
        os.mkdir(newdir)
        returnlist = []
        for item in sorted(os.listdir(dir)):
            if ( item[-4:] == '.png' or item[-4:]== '.PNG'):
                actualimage = image(dir+item)
                pygame.image.save(actualimage, newdir+'/'+item)
                returnlist.append(actualimage)
        return returnlist
