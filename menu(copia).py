...
#        if self.count > 80:
    ...
    def update_menus(self,surface):
        ...
        elif self.action == 'stayclosed':
            self.STEP = self.stay_closed_bar
        else:
    ...
    def stay_closed_bar(self,surface):
        if (-800 < self.bar.position[0] <10) or (self.universe.width-self.bar.size[0] < self.bar.position[0] < self.universe.width +1):
            self.bar.position[0] -= self.speed
            self.speed += 1
        else:
...
#        self.next_menu = self.watching_story
        self.screen.universe.LEVEL = 'stayclosed'
...
class Story_Frame():
    def __init__(self, image, menu):
        ...
        self.images         = [pygame.image.load(directory+i).convert_alpha() for i in frames]
        self.frame_number   = 0
        self.size   = self.image.get_size()
        self.pos    = []

    def update_all(self):
        self.menu.items = [self.images[i] for i in range(0,self.frame_number)]
